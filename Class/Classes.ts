...
class Person { /* O class serve para criar uma classe na qual pode ter atributos construtores e funções */
    name : string; // Este é um atributo, pois ele está na classe diferentemente de uma variavel quaquer
    
    constructor (name : string) { // O construtor inicializa a variavel (atributo) name com o nome passado
...
class Employee extends Person { // O extends informa que a clase Employee é um Person, ou sejá é uma herança. Basicamente todos os atributos e funções de Person são implementados em Employee
    salary : number;
    ...
    print () : void { // É possivel fazer a sobreescrita da função da classe pai. Só é preciso reescrever a função com o mesmo nome
...
let p3 = <Person> {}; // Está forma de cria uma pessoa sem precisar passsar os objetos necessarios (Igual acima)

let p4 : Person = new Employee ("Luffy", 2000); // Este é o chamado polimorfismo, o objeto é do tipo Employee mas a variavel de referencia é Person (Ou sejá só acessa os atributos de Person)
